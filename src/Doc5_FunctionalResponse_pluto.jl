...
using BioEnergeticFoodWebs, EcologicalNetworks, CSV, Random, Plots, DataFrames
...
The functional response is a classic concept in food web ecology and considers the relationship between predation rate and prey density. Specifically, it describes the number of prey that a predator consumes per unit of time as a function of the abundance or density of it's prey ([Solomon 1949](https://www.jstor.org/stable/1578), Holling 1959a). The functional response epitomizes the density-dependent nature of trophic interactions and therefore acts a key building block to several food web models including the BEFW. 

A predators functional response typically can take one of three types. In all three, consumption rate typically increases with prey density, albeit the exact shape of this increase and at what point at a maximum consumption rate is achieved will vary. Functional response types:
...
Type II functional responses are most frequently observed in the wild. For example, in a review of individual functional responses by [Jeschke et al. (2004)](https://onlinelibrary.wiley.com/doi/abs/10.1017/S1464793103006286) found that the frequency of type II functional responses (77%) far exceeded that of type III (13%) and type I (3%). Type II functional responses are also commonly used in food web models, however, type III's are also employed as they are known to boost species persistence by increasing ecological stability (because predation is low when prey densities are also low). 
"
