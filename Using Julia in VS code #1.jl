...
# (2) Open VS code - you'll see the VS code Welcome page
...
# (6) Navigate to your folder of choice and click open - this will then form the location of your directory (same as an RProject). Your directory will appear as a vertical pane on the left hand side of the screen. 
# (7) You can then create a new file (a script) using (cmd-N, File>New File or left click>New File in the directory) and save it with .jl extension (tells VS code you want to use the Julia language)
# (8) Open a new REPL (stands for - read, execute, print and loop) by either typing Alt-J Alt-O or by typing Start REPL in the command palette (accessed using F1, cmd-shift-P or View>Command Palette). The REPL is the console/terminal that you run Julia code in. 
# (9) Type 'Hello world' in your new script and run (ctrl-enter to run line by line or shift-enter to run a block of code):
println("Hello world")
