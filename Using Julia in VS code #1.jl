...
# (5) Open a new REPL (stands for - read, execute, print and loop) by either typing Alt-J Alt-O or by typing Start REPL in the command palette (accessed using F1, cmd-shift-P or View>Command Palette). The REPL is the console/terminal that you run Julia code in. 
...
# There are two ways to check taht you are actually working into the right environment: 
# - check the 'Julia env:...' on the bottom left of your screen, it should match your folder name
...
# Reading more about Julia's type system is actually a good idea to understand more about its behaviour: 
